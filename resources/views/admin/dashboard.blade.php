        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white  rounded-2xl shadow-sm border border-gray-100  overflow-hidden">
                <div class="p-6 border-b border-gray-100 ">
                    <h3 class="text-lg font-bold text-gray-800 ">Sekilas Tarif per Area</h3>
                        ...
                        @forelse($areasWithRates as $area)
                            <div class="bg-gray-50  rounded-xl p-4 border border-gray-200 ">
                                <div class="flex items-center mb-3">
                                    ...
                                    <span class="font-bold text-gray-700  capitalize">{{ $area->name }}</span>
                                </div>
                                    ...
                                    @else
                                        @foreach($area->rates as $rate)
                                            <div class="flex justify-between items-center text-sm p-2 bg-white  rounded shadow-sm">
                                                <span class="text-gray-600  font-medium">{{ ucfirst($rate->vehicle_type) }}</span>
                                                <span class="font-semibold text-purple-600">Rp {{ number_format($rate->amount, 0, ',', '.') }}</span>
                                            </div>
                                        @endforeach
                                    @endif
                                </div>
